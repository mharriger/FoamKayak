...
chineX = np.loadtxt('SeaBeeStations.csv', delimiter=',')
chineY = np.loadtxt('SeaBeeChine1Z.csv', delimiter=',')
chineZ = np.loadtxt('SeaBeeChine1Y.csv', delimiter=',')

chine3d = np.array((chineX, chineY, chineZ))
...
# Pick 3 points
p0 = np.array((chineX[1], chineY[0], chineZ[0]))
p1 = np.array((chineX[3], chineY[2], chineZ[2]))
p2 = np.array((chineX[5], chineY[4], chineZ[4]))
...
for i in range(len(chineY)):
    point = np.array((chineX[i+1],chineY[i],chineZ[i]))
...
for x in chineX[1:-1]:
    ...
    stations.append(np.array((global_to_local(po, np.array((-x, 0, zl)), xl, yl), global_to_local(po, np.array((-x, 0, zl)) - station_vec, xl, yl) * np.array((1, -100)))))
